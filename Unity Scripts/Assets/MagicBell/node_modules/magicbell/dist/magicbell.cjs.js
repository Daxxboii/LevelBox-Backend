...
function createHmac(secret, data) {
  if (!secret || !data)
    return "";
  if (!crypto__default.default || !crypto__default.default.createHmac) {
    throw new Error("This method is not available in the browser. Please provide a userHmac.");
  }
  return crypto__default.default.createHmac("sha256", secret).update(data).digest("base64");
}
function getEnvInfo() {
  var _a;
  const common = {
...
function createListener(client, args = {}) {
  ...
  async function connect(options) {
    ...
    const { token } = await axios__default.default.post(`https://rest.ably.io/keys/${auth.keyName}/requestToken`, auth).then((x) => x.data);
    await configPromise;
    ...
    eventSource = new EventSource__default.default(url.toString());
...
if (typeof XMLHttpRequest !== "function") {
  axios__default.default.defaults.adapter = require("axios/lib/adapters/http");
}
